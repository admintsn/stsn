                    ...
                    ->action(fn(Collection $records, array $data) => $records->each(
                        function ($record) {
                            ...
                            if ($cekdatats === 0) {
                                ...
                                $ta_s = TahunAjaran::where('abbr_ta', $getta->ta_s)->first();


                                $getsemaktif = TahunAjaranAktif::where('qism_id', $record->qism_id)->where('is_active', 1)->first();

                                $getsem = Semester::where('qism_id', $record->qism_id)->where('id', $getsemaktif->semester_id)->first();

                                $sem_s = Semester::where('qism_id', $record->qism_id)->where('abbr_semester', $getsem->sem_s)->first();
                                ...
                                $kelassantri->tahun_ajaran_id = $ta_s->id;
                                $kelassantri->semester_id = $sem_s->id;
                                ...
                                $kelassantri->walisantri_id = $datakelassantri->walisantri_id;
                                $kelassantri->kartu_keluarga = $datakelassantri->kartu_keluarga;
                                $kelassantri->save();
