...
use App\Filament\Tsn\Resources\Imtihan\PerkembanganAnakDidikResource\Pages;
use App\Filament\Tsn\Resources\Imtihan\PerkembanganAnakDidikResource\RelationManagers;
use App\Models\Nilai;
use App\Models\PerkembanganAnakDidik;
...
class PerkembanganAnakDidikResource extends Resource
{
    ...
    protected static ?int $navigationSort = 03030;

    protected static ?string $modelLabel = 'Nilai Rapor TA';

    protected static ?string $navigationLabel = 'Nilai Rapor TA';

    protected static ?string $pluralModelLabel = 'Nilai Rapor TA';
    ...
    {
        return [
            'index' => Pages\ListPerkembanganAnakDidiks::route('/'),
            'create' => Pages\CreatePerkembanganAnakDidik::route('/create'),
            'view' => Pages\ViewPerkembanganAnakDidik::route('/{record}'),
            'edit' => Pages\EditPerkembanganAnakDidik::route('/{record}/edit'),
        ];
    }
    ...
    public static function getEloquentQuery(): Builder
    {
        ...
        if (Auth::user()->id === 1 or Auth::user()->id === 2) {
            return parent::getEloquentQuery()->where('jenis_soal_id', 3)->where('is_nilai', 1)->where('tahun_berjalan_id', $tahunberjalan->id);
        } else {

            return parent::getEloquentQuery()->whereHas('pengajar', function ($query) {
                $query->where('user_id', Auth::user()->id);
            })->where('jenis_soal_id', 3)->where('is_nilai', 1)->where('tahun_berjalan_id', $tahunberjalan->id);
        }
    }
}
